...
### 2. 요약 단계에 "한국어 구조화 형식 가드" 추가

description이 없거나 한국어 구조화 형식이 아니면(=영문 원문 등 잘못 채워진 값) 재요약 대상으로 본다. 이로써 기존에 영문 description으로 저장된 article도 자동으로 정상 복구된다.
...
가드는 `## 주요 내용` 또는 `## 시사점` 둘 중 하나만 있어도 정상으로 본다(모델이 종종 `## 개요`를 생략하기 때문). regex로 공백 변동을 허용한다.
