...
| 2026-04-21 | **SSR 경로의 like 토큰 누락 해결 (근본 원인)** — PR #140에서 `src/styles/global.css`에 `--color-like-*` 토큰 10개를 추가했지만, Cloudflare Functions SSR 페이지(`functions/p/[id].ts`, `functions/trending.ts`)가 사용하는 별도의 `functions/lib/layout.ts :: BASE_STYLES`에는 누락된 상태. 결과 라이브 `/p/QpPyDksJqZnb`에서 `var(--color-like-default-icon)` 같은 참조가 모두 빈 값으로 평가되어 아이콘이 `#2F2B31`(진한 회갈색)으로 fallback, 테두리는 parse fail로 비가시화되던 문제. Oracle 8차 검증이 Playwright `getComputedStyle(document.documentElement).getPropertyValue('--color-like-*')`로 직접 빈 문자열임을 증명. `functions/lib/layout.ts`의 BASE_STYLES `:root`에 동일한 like 토큰 10개를 추가하고 "MUST stay in sync with src/styles/global.css" 주석 명시. PR #131~#147 시리즈가 놓친 이중 렌더링 경로의 토큰 공급체인 기존 구조를 마지막으로 정리. |
