...
> 기술 분야 주요 인물들의 프로필을 플랫폼별(X, Threads, YouTube)로 모아 보여주는 디렉토리 기능.

## 개요

기술/AI/Web3 분야 인플루언서들을 카드 형태로 소개하고, 각 프로필 링크를 통해 바로 팔로우할 수 있도록 한다. X (Twitter), Threads, YouTube 섹션을 분리하여 플랫폼별로 탐색할 수 있다.
...
src/data/influencers/*.json (개별 데이터 파일 — Content Collection 소스)
  ...
  → /influencers/ 페이지 렌더링 (X / Threads / YouTube 섹션 분리)
```

### 페이지 렌더링

1. `influencers.astro`가 Astro Content Collection에서 전체 인플루언서 데이터 로드
2. `platform` 필드 기준으로 X(`x`), Threads(`threads`), YouTube(`youtube`) 그룹 분리
...
- YouTube: `https://www.youtube.com/@{handle}` (@ 제거)

### 데이터 검증

- Zod 스키마(`src/schemas/influencer.ts`)로 Astro Content Collection 빌드 시 유효성 검사
- `scripts/validate.ts`는 curated/feed 컨텐츠만 검증하며, 인플루언서 데이터는 Astro 빌드 시 자동 검증된다

## 관련 파일

| 파일 | 역할 |
|------|------|
| `src/pages/influencers.astro` | 인플루언서 목록 페이지 (X/Threads/YouTube 섹션 분리) |
...
| ID | 이름 | 핸들 | 분야 |
|----|------|------|------|
| `chester-roh` | 노정석 | @chester_roh | AI 창업/테크 팟캐스트 |
| `code-factory` | 코드팩토리 | @codefactory_official | Flutter/React/AI 코딩 교육 |
| `eo-korea` | EO Korea | @eo_korea | 한국 스타트업 인터뷰 |
| `gymcoding` | 짐코딩 | @gymcoding | 웹 개발/React/Next.js 교육 |
| `today-code` | 오늘코드 | @todaycode | Python 데이터 분석/NLP |
...
- 플랫폼은 `x`, `threads`, `blog`, `youtube` 중 하나만 지원 (현재 `x`, `threads`, `youtube` 활성; `blog` 미활성)
- bio 최대 300자
- 데이터는 수동 관리 — 자동 수집 없음
...
| 2026-04-21 | YouTube 섹션 신설 — 큐레이터 5명(노정석, 짐코딩, EO Korea, 오늘코드, 코드팩토리)을 curated 기사에서 인플루언서로 이동 (총 16명). `InfluencerCard.astro`에 YouTube 프로필 URL 생성 로직 추가, `influencers.astro`에 YouTube 섹션 추가 |
