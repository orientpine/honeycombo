...
전체 기사에서 수집한 수백 개의 태그(작성 당시 기준 ~241개, 이후 대량 증가)가 필터 UI에 표시되지만, 실제로 현재 페이지에 해당 태그 기사가 없으면 0건이 노출되어 "기사에 없는 태그"처럼 보이는 문제가 있었다.
...
- 인라인 JSON 크기는 기사 수에 비례 (작성 당시 기준 ~244건 규모 ≈ 수백 KB raw, gzip 후 수십 KB)
