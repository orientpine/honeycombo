...
> 에디터가 직접 필독 기사를 선정·관리하는 기능. D1 기반 저장, SSR 렌더링.
...
- **공개**: `/must-read` SSR 페이지에서 D1 데이터 기반 렌더링
...
```
방문자 → /must-read → functions/must-read.ts (SSR)
  → D1 must_read_items 조회 (position 순)
  → HTML 렌더링 (renderDocument 사용)
```
...
| `functions/must-read.ts` | SSR 공개 페이지 |
