...
docs/**/*.md 스캔 → 유형별 필수 섹션 검증 → 결과 출력 (❌/✅)
                                              ↓
                               (--check-coverage) git diff → 코드 변경 + docs 미변경 시 ⚠️ 경고
```
...
### 변경 커버리지 감지 (경고만, CI 빌드 계속)

`--check-coverage` 플래그 사용 시:
- `src/`, `functions/`, `scripts/` 변경이 있는데 `docs/` 변경이 없으면 경고
- 자동 생성 데이터(`src/data/feeds`, `src/data/trending`, `src/data/must-read`, `src/data/influencers`)는 제외
...
| `EXCLUDED_DATA_DIRS` | `scripts/validate-docs.ts` | feeds, trending, must-read, influencers | 자동 생성 데이터 디렉토리 (커버리지 검사 제외) |
