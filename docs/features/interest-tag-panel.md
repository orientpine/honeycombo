...
| 2026-04-21 | **필터 결과 헤더 재디자인** — 관심사/태그 칩을 클릭했을 때 표시되는 `"tech" 태그 · 기사 213건` 텍스트가 기사 그리드와 너무 가까워 어색하다는 피드백 대응. (1) `.itp-filtered-results` 과 `.itp-filtered-header` 사이 간격을 `var(--space-md)` → `var(--space-2xl)`(데스크탑) / `var(--space-xl)`(모바일)로 확대. (2) `buildFilterLabel()`(단일 텍스트 반환)을 `buildFilterLabelHTML()`(구조화 HTML 반환)로 교체 — 활성 소스/태그/검색어를 각각 시각 차별한 pill로, 기사 카운트는 우측 정렬된 강조 수치 배지로 표시. (3) `.itp-filtered-header`에 디자인 시스템 토큰 기반 시각 강조 추가 — 좌측 primary accent bar(3px), `--color-bg-secondary` 배경, `--shadow-sm`, 둘근 레디어스. 태그 pill은 primary 배경 + 부드러운 섬도우. (4) Empty 상태(`조건에 해당하는 기사가 없습니다`)도 dashed 테두리 + 배경 적용해 일관된 느낌으로 재디자인. (5) 모바일에서는 카운트 배지를 하단으로 줄바꿔 세로 접힌 구조로 대응. 색상은 CSS custom property(`var(--color-*)`)를 사용해 하드코딩 최소화 — 현재 사이트는 라이트모드 전용이며(`color-scheme: light`), 다크모드는 도입되지 않은 상태. 수정 파일: `src/components/InterestTagPanel.astro` (`buildFilterLabelHTML` 신규, `renderFilteredView`에서 `textContent` → `innerHTML`, 스타일 명드 확장). |
| 2026-04-21 | **헤더 구조 버그 수정 (Oracle 검증 후속)** — 이전 커밋(#132)에서 `buildFilterLabelHTML()`가 생성하는 `.itp-filtered-pills`/`.itp-filtered-count-badge`를 기존 인라인 `<span id="itp-filtered-count">` 안에 넣어 출력했던 구조적 버그 해결. 이로 인해 `.itp-filtered-count-badge`의 `margin-left: auto`(데스크탑 우측 정렬)와 `width: 100%`(모바일 줄바꿈) flex-item 스타일이 **실제로 적용되지 않았다** (인라인 부모는 flex 컨텍스트를 제공하지 않음). 해결: 마크업을 `<div class="itp-filtered-header" id="itp-filtered-count">`로 단순화해 헤더 컨테이너(flex) === innerHTML 타겟 컨테이너로 통합. 이제 grandchild다 직접 자식이 되어 `margin-left: auto`/`width: 100%`가 의도대로 동작. 추가로 `.itp-filtered-pill-tag`의 하드코딩 `#FFFFFF`를 `var(--color-bg)`로 교체(AGENTS.md 색상 하드코딩 금지 규칙 준수). 수정 파일: `src/components/InterestTagPanel.astro`. |
| 2026-04-21 | **하드코딩 그림자 완전 제거 (Oracle 2차 검증)** — `.itp-filtered-pill-tag`에 남아 있던 `box-shadow: 0 1px 3px rgba(245, 124, 34, 0.25)` 하드코딩 RGBA를 제거하기 위해 `--shadow-primary-soft`와 `--shadow-primary-lift` 토큰을 `src/styles/global.css`에 신규 추가. `.itp-filtered-pill-tag`의 shadow를 `var(--shadow-primary-soft)`로 교체. AGENTS.md "색상 하드코딩 금지" 규칙 완전 준수. 수정 파일: `src/styles/global.css`(토큰 2개 추가), `src/components/InterestTagPanel.astro`(shadow 토큰 교체). |
| 2026-04-21 | **태그 중복 표시 제거 + 원본 텍스트 포맷 복원 (Oracle 3차 검증)** — `.itp-selected-summary`가 이미 활성 태그를 삭제 가능한 pill로 표시하고 있으며, 새로 만든 결과 헤더가 또 한번 `#tech` pill로 표시하는 것은 시각 중복(visual noise)이고, 사용자가 말한 `"tech" 태그 · 기사 213건` 원본 표현을 해시태그 스타일(`#tech`)로 바꿔버린 시맨틱 드리프도 발생. 해결: `buildFilterLabelHTML()`을 **자연어 디스크립터 형식**으로 재설계 — `"tech" 태그`(단일) / `태그 3개(ai, llm, tech)`(다중) / `에디터 추천 · "ai" 태그 · "검색어" 검색`(조합) 포맷으로 사용자 원본 표현 존중. 카운트 `<strong>N</strong>건의 기사`만 primary 색으로 강조(단일 accent). `.itp-filtered-pill*` 관련 CSS 및 사용처 없어진 `--shadow-primary-*` 토큰 제거(YAGNI). 수정 파일: `src/styles/global.css`(사용처 없는 토큰 2개 제거), `src/components/InterestTagPanel.astro`(buildFilterLabelHTML 재설계, pill CSS 제거, descriptor CSS 추가). |
