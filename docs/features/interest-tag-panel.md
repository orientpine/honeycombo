...
| 2026-04-21 | **필터 결과 헤더 재디자인** — 관심사/태그 칩을 클릭했을 때 표시되는 `"tech" 태그 · 기사 213건` 텍스트가 기사 그리드와 너무 가까워 어색하다는 피드백 대응. (1) `.itp-filtered-results` 과 `.itp-filtered-header` 사이 간격을 `var(--space-md)` → `var(--space-2xl)`(데스크탑) / `var(--space-xl)`(모바일)로 확대. (2) `buildFilterLabel()`(단일 텍스트 반환)을 `buildFilterLabelHTML()`(구조화 HTML 반환)로 교체 — 활성 소스/태그/검색어를 각각 시각 차별한 pill로, 기사 카운트는 우측 정렬된 강조 수치 배지로 표시. (3) `.itp-filtered-header`에 디자인 시스템 토큰 기반 시각 강조 추가 — 좌측 primary accent bar(3px), `--color-bg-secondary` 배경, `--shadow-sm`, 둘근 레디어스. 태그 pill은 primary 그라디언트 + 부드러운 섬도우. (4) Empty 상태(`조건에 해당하는 기사가 없습니다`)도 dashed 테두리 + 배경 적용해 일관된 느낌으로 재디자인. (5) 모바일에서는 카운트 배지를 하단으로 줄바꿔 세로 접힌 구조로 대응. 모든 색상은 CSS custom property(`var(--color-*)`)를 사용해 다크모드 호환. 수정 파일: `src/components/InterestTagPanel.astro` (`buildFilterLabelHTML` 신규, `renderFilteredView`에서 `textContent` → `innerHTML`, 스타일 명드 확장). |
