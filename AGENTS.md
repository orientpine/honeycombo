...
| **1** | DESIGN.md가 **작업에 영향을 받는지 검토**한다 — 새 색·새 간격·새 그림자·새 컴포넌트·새 레이아웃 패턴·새 폰트·새 브레이크포인트·새 상태(hover 변형 등)·새 interaction 패턴·다크모드 등이 **하나라도 필요하면 단계 2A → 2B**, 없으면 단계 3으로 직행. 애매하면 **변경하는 쪽** (즉 DESIGN.md를 먼저 명확히 하는 쪽)으로 판단한다. |
...
**핵심 규칙 (리터럴):** *변경이 필요하면 “항상 DESIGN.md를 먼저 편집한다.”* 코드보다 문서가 먼저이고, 문서에 없는 새 패턴을 코드에 먼저 들이는 PR 은 리뷰에서 반돌된다.

### 이 규칙이 엄격한 이유

- 현재 스냅샷 기준 DESIGN.md와 실제 코드가 일치하도록 유지하지 않으면, 밍고 돌아갈 agent가 엄륩한 패턴을 정당심으로 확산시키고, 문서는 좌빨간에 루즈해진다.
