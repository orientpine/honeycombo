...
| Button (form/admin) | `.btn` (forms) | `0.95rem` | `600` | default | `padding: var(--space-sm) var(--space-lg)`, `border-radius: var(--radius-md)`, `transition: all 0.15s` | `pages/p/new.astro:425-437`, `pages/admin/must-read.astro:565-579` |
...
| 2026-04-21 | 배치 편집 드래그 **자동 스크롤** 활성화 — 긴 플레이리스트에서 드래그 중 페이지 자동 스크롤을 동작시키는 동작 개선. SortableJS `scroll`/`scrollSensitivity: 80`/`scrollSpeed: 20`/`bubbleScroll: true`/`forceAutoScrollFallback: true` 옵션만 추가 — **신규 토큰·UI 패턴·CSS 변경 없음**. §4.9 Batch Edit Mode 동작 강화만에 해당하여 변경 이력만 기록. |
| 2026-04-21 | 배치 편집 드래그 자동 스크롤 속도 **2배 증가** — SortableJS `scrollSpeed: 20 → 40` (프레임당 40px, 60fps 기준 약 2,400px/s). 어지러운 속도 피드백에 따른 튜닝 — 신규 토큰·UI 패턴·CSS 변경 없음. §4.9 동작 수치 조정만에 해당. |
